...
import { calculateDecisionSimulation } from '../shared/runwayCalculator';
...
export interface ChatRequestBody {
  messages: Array<{ sender?: string; role?: string; content?: string; text?: string }>;
  userContext?: {
    currency?: string;
    region?: string;
...
export async function processChatRequest(body: ChatRequestBody): Promise<any> {
  const { messages, userContext, simulation } = body || {};
...
Which option fits your current priorities best? Would you like to log this movement or simulate another amount?`;
    ...
    // 7. Purchase / Subscription / Outing Intent WITHOUT Price
...
What financial decision or project would you like to explore together today?`;
    ...
    // 9. Unrecognized / Ambiguous Query Fallback
    if (isFrench) {
...
Could you share a bit more detail about what you'd like to achieve?`;
  ...
  if (!ai) {
    return generateLocalNeyAnalysis();
  }
...
1. APP HOW-TO & NAVIGATION: Explain the step-by-step navigation in the app clearly.
2. FINANCIAL AFFORDABILITY & EXPENSE SIMULATIONS: Categorize with [SCENARIO: GO], [SCENARIO: WAIT], [SCENARIO: ADJUST], or [SCENARIO: NO]. Quantify exact runway impact.
3. PROJECT CAPTURE & DYNAMIC SCORING: Extract title, description, and 4 variables (revenue_potential, validated_demand, urgency, estimated_time on scale 1-10), compute score = (revenue*demand*urgency)/time, and output clean JSON ready for Firestore.
4. ASKING FOR CLARIFICATION: If price/frequency is missing, state assumptions politely and ask for details.
...
Keep responses concise, scannable, warm, conversational, and highly structured with bullet points and bold highlights.`;
  ...
  for (const m of rawList) {
    const text = (m.content || (m as any).text || '').trim();
    if (!text) continue;
  ...
  try {
    ...
    } else if (/project|projet|idea|idée|startup|score|priorit/i.test(lowerLatest)) {
      followUpOptions = isFrench ? [
